...
gulp.task('themeToConstant', function () {
  return gulp.src(paths.src+'expander-theme.scss')
    .pipe(sass())
    .pipe(through2.obj(function (file, enc, cb) {
      var config = {
        ...
        deps: false,
        constants: {
          EXPANDER_THEME: file.contents.toString()
        }
      };
...
gulp.task('buildIconCacheDev', function () {
  return gulp.src(paths.icons)
    .pipe(templateCache({module: 'material.components.expander'}))
    .pipe(gulp.dest(paths.public));
});
...
gulp.task('watch', function () {
  ...
  gulp.watch('src/exapnder-theme.scss', ['themeToConstant']);
